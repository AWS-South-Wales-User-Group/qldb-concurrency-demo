...
const TransactionExecutionContext_1 = require("./TransactionExecutionContext");
/**
...
class QldbDriver {
    ...
    constructor(ledgerName, qldbClientOptions = {}, maxConcurrentTransactions = 0, retryConfig = DefaultRetryConfig_1.defaultRetryConfig) {
        this._returnSessionToPool = (session) => {
            ...
            LogUtil_1.debug(`Session returned to pool; size is now ${this._sessionPool.length}.`);
        };
    ...
    executeLambda(transactionLambda, retryConfig) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const transactionExecutionContext = new TransactionExecutionContext_1.TransactionExecutionContext();
            let transactionExecutionAttempt = 0;
            while (true) {
                try {
                    transactionExecutionAttempt += 1;
                    session = yield this.getSession();
                    return yield session.executeLambda(transactionLambda, retryConfig, transactionExecutionContext);
                }
                catch (err) {
                    ...
                    */
                    ...
                    if (Errors_1.isInvalidSessionException(err) && !Errors_1.isTransactionExpiredException(err)) {
                        continue;
                    }
                    else {
                        throw err;
    ...
    getSession() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const isPermitAcquired = this._semaphore.tryAcquire();
            if (isPermitAcquired) {
                this._availablePermits--;
                try {
                    let session = this._sessionPool.pop();
                    if (session == undefined) {
                        LogUtil_1.debug("Creating new pooled session.");
                        session = (yield this._createSession());
                    }
                    return session;
                }
                catch (e) {
            ...
            throw new Errors_1.SessionPoolEmptyError(this._timeoutMillis);
        });
    }
    _throwIfClosed() {
        if (this._isClosed) {
    ...
    _createSession() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const communicator = yield Communicator_1.Communicator.create(this._qldbClient, this._ledgerName);
